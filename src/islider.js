...
import isliderAnimate from './ext/islider_animate.js';
...
class iSlider {
    ...
    _setting() {
         ...
         */
        this.animateEasing =
            inArray(opts.animateEasing, this.EASING[0]) || this.EASING[1].test(opts.animateEasing) ? opts.animateEasing : 'ease';
    ...
    _renderItem(el, dataIndex) {
        ...
        let insertImg = function renderItemInsertImg() {
    ...
    slideTo(dataIndex, opts) {
        ...
        let animateType = this.animateType;
     ...
     * @public
     */
    unbind(evtType, selector, callback) {
        let key = evtType + ';' + selector;
        if (this._EventHandle.hasOwnProperty(key)) {
            let i = this._EventHandle[key][0].indexOf(callback);
            if (i > -1) {
    ...
    unDelegate(evtType, selector, callback) {
        let key = evtType + ';' + selector;
        if (this._EventHandle.hasOwnProperty(key)) {
            let i = this._EventHandle[key][0].indexOf(callback);
            if (i > -1) {
    ...
    destroy() {
        ...
        for (let n in this._EventHandle) {
            let handList = this._EventHandle[n][1];
    ...
    off(eventName, func) {
        let index = this.has(eventName, func);
        if (index > -1) {
